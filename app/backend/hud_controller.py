...
from .fleet_service import FleetDataService

CONFIG_PATH = os.path.expandvars(r'%LOCALAPPDATA%\hermes\dynamic_flow_config.json')
...
class HUDController(QObject):
    ...
    def __init__(self):
        ...
        self.fleet_service = FleetDataService()
        ...
        # Fleet state properties
        self.fleet_state = "IDLE"
        self.active_agent_name = "Fleet Idle"
        self.active_tool_name = ""
        self.fleet_tokens_str = "0"
        self.fleet_reasoning_str = "0"
        self.fleet_active_count = 0
        self.fleet_total_count = 0
        self.fleet_agent_list = []
    ...
    def _refresh_fleet(self):
        try:
            snapshot = self.fleet_service.fetch_fleet_snapshot()
            old_active = self.fleet_state == "BUSY"
            self.fleet_state = snapshot.state
            new_active = self.fleet_state == "BUSY"
            self.fleet_active_count = len(snapshot.active_agents)
            self.fleet_total_count = len(snapshot.agents)
            self.active_tool_name = snapshot.active_tool or ""

            if snapshot.active_agents:
                primary = snapshot.active_agents[0]
                tool_suffix = f" · {self.active_tool_name}" if self.active_tool_name else ""
                self.active_agent_name = f"@{primary}{tool_suffix}"
            else:
                self.active_agent_name = "Fleet Idle"

            toks = snapshot.total_tokens_today
            if toks >= 1_000_000:
                self.fleet_tokens_str = f"{toks / 1_000_000:.1f}M"
            elif toks >= 1_000:
                self.fleet_tokens_str = f"{toks / 1_000:.1f}k"
            else:
                self.fleet_tokens_str = str(toks)

            agent_cards = []
            for name, info in snapshot.agents.items():
                r_str = f" ({info.reasoning_tokens:,} r)" if info.reasoning_tokens > 0 else ""
                agent_cards.append({
                    "name": f"@{name}",
                    "model": info.model,
                    "isActive": info.is_active,
                    "task": info.latest_task,
                    "tokens": f"{info.today_tokens:,}{r_str}",
                    "timeAgo": info.time_ago_str
                })
            self.fleet_agent_list = sorted(agent_cards, key=lambda x: (not x["isActive"], x["name"]))
    ...
    def poll_tick(self):
        self.refresh_stats()
        if self._current_view == 'detailed':
            self.check_health()
        self._refresh_fleet()
    ...
    @Property(str, notify=statsChanged)
    def fleetTokens(self):
        return self.fleet_tokens_str

    @Property(str, notify=statsChanged)
    def fleetReasoning(self):
        return self.fleet_reasoning_str

    @Property(int, notify=statsChanged)
    def fleetActiveCount(self):
        return self.fleet_active_count

    @Property(int, notify=statsChanged)
    def fleetTotalCount(self):
        return self.fleet_total_count

    @Property('QVariantList', notify=statsChanged)
    def fleetAgents(self):
        return self.fleet_agent_list
